...
const { configureChains, createConfig, getNetwork, disconnect, getAccount, watchNetwork, watchAccount, readContract } = WagmiCore;
...
async function onDisconnect() {
	let account = null;
	const { disconnect, getAccount, watchAccount } = WagmiCore;
	account = getAccount();
...
async function fetchAccount() {
	...
	if(network.chain){
		...
		async function rowResolvers() {
			...
			const { fetchBalance, fetchToken, readContract, getNetwork } = WagmiCore;
		...
		if (getScheduleCount == 0) { 
			$(".tokenClaim").css({"display":"none"}); 
		} else { 
			$(".tokenClaim").css({"display":"block"}); 
		}
		...
		while (i < getScheduleCount) {
			try {
				...
				else { var startTime = Number(getSchedule.start);
				}
				...
				if(releasableAmountDec < 0){ releasableAmountDec = 0; }
				var disabled = "";
...
async function init() {
	...
	const { getAccount, watchAccount, watchNetwork, getNetwork, switchNetwork } = WagmiCore;
